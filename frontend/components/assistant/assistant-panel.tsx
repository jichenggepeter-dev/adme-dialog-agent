...
export function AssistantPanel() {
  ...
  return <><AssistantLauncher />{open ? <aside className={`assistant-panel assistant-panel-docked ${closing ? "is-closing" : ""} action-${actionPhase}`} aria-label="ADME Assistant" role="complementary">
    ...
    <div className="assistant-context"><div><span>Current context</span><strong>{batch ? "Batch Screening" : pathname.startsWith("/about") ? "Model Information" : "Single Molecule"}</strong></div><SessionDeletionControls sessionId={sessionId} stateVersion={stateVersion} disabled={!ready || loading} onDelete={deleteCurrentSession} /></div>
