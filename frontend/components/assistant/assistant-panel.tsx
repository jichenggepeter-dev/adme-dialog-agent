...
export function AssistantPanel() {
  const { open, closing, setOpen, ready, loading, sessionId, stateVersion, messages, pending, pendingAction, error, streamStatus, actionPhase, actionResult, guidedPrediction, mockScenario, setMockScenario, send, cancelStream, decide, decideAction, deleteCurrentSession, clearError } = useAssistant();
  const [draft, setDraft] = useState(""); const messageListRef = useRef<HTMLDivElement>(null); const composerRef = useRef<HTMLTextAreaElement>(null); const closeRef = useRef<HTMLButtonElement>(null); const pathname = usePathname();
  ...
  function closePanel() { setOpen(false); requestAnimationFrame(() => document.querySelector<HTMLButtonElement>("[data-assistant-launcher]")?.focus()); }
  return <><AssistantLauncher />{open ? <aside id="adme-assistant-panel" className={`assistant-panel assistant-panel-docked ${closing ? "is-closing" : ""} action-${actionPhase}`} aria-label="ADME Assistant" role="complementary">
    <header className="assistant-header"><div><span className="assistant-mark"><ChatCircleDots size={20} weight="fill" /></span><div><strong>ADME Assistant</strong><small><i /> {MOCK_AGENT_MODE ? "Mock Agent v1 · deterministic review" : "Scientific workspace copilot"}</small></div></div><button ref={closeRef} className="icon-button" aria-label="Close Assistant" onClick={closePanel}><X size={19} /></button></header>
    <div className="assistant-context"><div><span>Current context</span><strong>{batch ? "Batch Screening" : pathname.startsWith("/about") ? "Model Information" : "Single Molecule"}</strong></div><div className="assistant-session-controls"><SessionExportControls sessionId={sessionId} stateVersion={stateVersion} disabled={!ready || loading} /><SessionDeletionControls sessionId={sessionId} stateVersion={stateVersion} disabled={!ready || loading} onDelete={deleteCurrentSession} /></div></div>
    ...
    <div className="assistant-messages" role="region" aria-label="Assistant conversation" ref={messageListRef}>
